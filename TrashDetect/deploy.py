...
def deploy_sample(model, dataset, index, device, threshold=0.5):
    ...
    for box, label in zip(target["boxes"], target["labels"]):
        ...
        cv2.putText(image_np, f"GT:{name}", (x1, y1 - 5),
    ...
    for box, score, label in zip(boxes, scores, labels):
        ...
        cv2.putText(image_np, f"{name} {score:.2f}", (x1, y2 + 15),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (0, 255, 0), 1)

    # Show kết quả
    plt.figure(figsize=(10, 10))
    plt.imshow(cv2.cvtColor(image_np, cv2.COLOR_BGR2RGB))
    plt.title(f"Sample Index: {index} (Red: GT, Green: Pred)")
    plt.axis("off")
    plt.show()
...
def main():
    ...
    dataset = TrashDataset(
        root=DATA_PATH,
        split="train",
        transforms=get_test_transform(IMAGE_SIZE)
    )
